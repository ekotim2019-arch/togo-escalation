  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate">
      ...
      >
        ...
        <h2 className="text-3xl font-bold text-paper">The {BRAND.name} Difference</h2>
      </motion.div>
      ...
      <div className="flex-1 flex flex-col px-16 pb-16 gap-6 min-h-0">
        ...
        >
          ...
          <div className="grid grid-cols-[140px_1fr_1fr] gap-2 mb-2">
            <div />
            <div className="text-center bg-ash/10 rounded-t-lg py-2 px-4">
              <span className="text-ash text-sm font-bold tracking-widest uppercase">Conventional Intel</span>
            </div>
            <div className="text-center bg-amber/15 rounded-t-lg py-2 px-4 border-t border-amber/30">
              <span className="text-amber text-sm font-bold tracking-widest uppercase">{BRAND.name}</span>
            </div>
          </div>
          ...
          <div className="space-y-1">
            {comparison.map((row, i) => (
              ...
              >
                ...
                <div className="bg-ash/5 rounded-lg py-3 px-4 text-paper/60 text-sm self-center">
                  {row.conventional}
                </div>
                <div className="bg-amber/10 border border-amber/20 rounded-lg py-3 px-4 text-paper text-sm font-medium self-center">
                  {row.kairos}
                </div>
              </motion.div>
            ))}
        ...
        >
          <div className="text-ash text-xs font-bold tracking-widest uppercase mb-3">Intelligence Pipeline</div>
          <div className="flex items-center gap-2">
            {pipeline.map((step, i) => (
              <div key={step.step} className="flex items-center gap-2 flex-1">
                <div className="flex-1 rounded-lg border px-3 py-2 text-center" style={{
                  borderColor: `${step.color}40`,
                  backgroundColor: `${step.color}10`,
                }}>
                  <div className="text-xs font-bold" style={{ color: step.color }}>{step.step}</div>
